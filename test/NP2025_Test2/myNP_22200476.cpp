...
void odeEU_IM(double y[], double odeFunc(const double t, const double y), const double t0, const double tf, const double h,
	const double y_init)
{
	// Euler's Implicit Method
	// 미래값(모르는값) -> 미래값(모르는값) 추정
	// 미분된 함수, 초기값 필요
	...
	const double tf, const double h, const double y1_init, const double y2_init)
{
	...
	double yi[2] = { 0 };	// yi[0] = 현재y(i), yi[1] = 현재v(i) -> K1 만들때 필요함
...
/*==========================================================================*/
// linear curve fitting (1st order polynomial)
void linearRegression_1D(double z_opt[], double xdata[], double ydata[], int dataN) {
	// 1. Initialization	
	double Sx = 0;
	double Sxx = 0;
	double Sxy = 0;
...
// higher order polynomial curve-fitting -> myMatrix
