...
{
	// v1 = transpose(v2), (1xn) vector,  v2 = (nx1) vector
...
// Forward Substitution  (Ly=b)      --- index 조심!!
...
// Back Substitution  (Ux=y)	  --- index 조심!!
...
// ------- (2D) - higher-order polynomial curve-fitting -----------------
Matrix highPolyFit(Matrix _vecX, Matrix _vecY, int n) { ///*********************************

	// initialize
	int m = _vecX.rows;  // size of Xdata
	int my = _vecX.rows;
	...
	Matrix Zopt = zeros(n + 1, 1);

	Zopt = zeros(n + 1, 1);

	// 2. Is length(X)~= length(Y) ? Exit : Continue
	if (m != my) {
		printf("ERROR: The number of elements in x must be the same as in y.\n");
	}
	// 3. Is m < (n + 1) or n < 1 ? Exit : Continue
	if (m < (n + 1) || n < 1) {
		printf("the condition does not meet!\n");
		return zeros(0, 0);
	}

	// 4. Calculate Sx(i) = sum(xk ^ (i)), for i = 0 to 2n  #(2n + 1) and k = 1 to m
	double Sx_temp = 0;
	...
	// 5. Construct matrix S with Sx(i).
	int k = 0;
	...
	// 7. Calculate optimal Z = inv(S) * (b)
		// where Z = [a0 a1....an]:  (n + 1)x1
	Matrix invS = zeros(S.rows, S.cols);
	invMat(&S, &invS);
	Zopt = multMat(invS, b);// Zopt = inv(S) * b;

	return Zopt;
}


// ------- (3D data) - planar curve fitting -----------------------
Matrix planarFitting(Matrix _vecX, Matrix _vecY)
{

	int m = _vecX.rows;  // size of Xdata
	int d = _vecX.cols;	// d:dimension
	Matrix A = ones(m, d + 1);      // 1열:x1[], 2열:x2[], 3열:y[]
	Matrix Z = zeros(d + 1, 1);

	for (int i = 0; i < m; i++) {
		A.at[i][1] = _vecX.at[i][0];
		A.at[i][2] = _vecX.at[i][1];
	}
...
void gradientDescnt(double lossFunc(Matrix Z, Matrix X, Matrix Y), Matrix lossGrad(Matrix Z, Matrix X, Matrix Y), 
	Matrix Xdata, Matrix Ydata)
{
		// Variables
	Matrix W = zeros(2, 1);
	...
	Matrix L = arr2Mat(L_raw, 6, 1);
	Matrix V = arr2Mat(V_raw, 6, 1);
	Matrix L_norm = normalizeMat(L);
	//Matrix V = copyMat(V);

	printMat(L_norm, "Lmat");
	printMat(V, "Vmat");
	...
	for (int k = 0; k < itrMax; k++) {
		...
		grad_L = lossGrad(W, L, V);

		W.at[0][0] = W.at[0][0] - eta * grad_L.at[0][0];
		W.at[1][0] = W.at[1][0] - eta * grad_L.at[1][0];

		// Calculate Loss
		loss = lossFunc(W, L_norm, V);
		printf("iter =%d \t w0=%0.3f \t w1=%0.3f \t L(x)=%0.3f\n", k, W.at[0][0], W.at[1][0], loss);
	}


	// Unormalize
	W_est = unnormMat(W, L_norm);
...
Matrix nonlinearSys(Matrix Funcs(Matrix _Z), Matrix Jacob(Matrix _Z), Matrix _Z0, double tol) {
	...
	for (int k = 0; k < 20; k++) {
		...
		// loss = norm2(F);   pow(F.at[0][0], 2.0) + pow(F.at[0][1], 2) + ...
...
// 얘는 단순히 출력 개수 달라서 한개 더 만든것 뿐입니다
Matrix nonlinearSys2(Matrix Funcs(Matrix _Z), Matrix Jacob(Matrix _Z), Matrix _Z0, double tol) {
	...
	for (int k = 0; k < 20; k++) {
		...
		printf("iter =%d \t th=%0.3f \t x=%0.3f \t y=%0.3f \t loss=%0.3f\n", k, Z.at[0][0], Z.at[1][0], Z.at[2][0], loss);
